...
//a função 'exibirResultados' receber um array de objetos e retorna uma tabela sem poluição como no 'console.table()'
function exibirResultados(input) {
...
//const result serve para tranformar o array 'contabilizar' em um array de objetos, afim servir como entrada a função 'exibirResultados'
const result = [
...
console.clear();
console.log("Confira resultado da votação: ");
exibirResultados(result);

result.splice(3,2);//retirando brancos e nulos do array para facilitar a definição do vencedor
...
//estrutura que define o vencedor da votação:
  if(result[0].Votos == result[1].Votos && result[1].Votos == result[2].Votos){
    console.log(`Os tres Candidatos empataram com ${result[0].Votos} votos.`);    
  }else if(result[0].Votos == result[1].Votos){
    console.log(`O ${result[1].Candidato} e ${result[0].Candidato} empataram com ${result[0].Votos} votos.`);
  }else{
      console.log(`O grande vencedor foi o ${result[0].Candidato} com ${result[0].Votos} votos!`);
  }